...
function taskFor(source, reviewer='demo'){
  return {schema_version:1,task_id:`container-${Date.now()}`,intent:'Change src/value.txt',source:{kind:'local_git',path:source,ref:execFileSync('git',['rev-parse','HEAD'],{cwd:source,encoding:'utf8'}).trim()},workers:{builder:{adapter:'container',image:nodeImage,command:'node',args:['-e',"require('fs').writeFileSync('/workspace/src/value.txt','after\\n');console.log('changed')"]},reviewer:reviewer==='demo'?{adapter:'demo'}:{adapter:'container',image:nodeImage,command:'node',args:['-e',"const fs=require('fs');const p=process.argv.at(-1)||'';const sha=p.match(/Candidate commit: ([^\\n]+)/)?.[1]?.trim();const tree=p.match(/Candidate tree: ([^\\n]+)/)?.[1]?.trim();fs.writeFileSync('/workspace/src/reviewer-write.txt','container-only');console.log(JSON.stringify({decision:'APPROVE',reason:'disposable-copy-check',residual_risks:[],reviewed_candidate_sha:sha,reviewed_tree_hash:tree}))"]}},allowed_actions:['build_local','merge'],protected_actions:['merge'],allowed_paths:['src'],budget:{model_calls:4,wall_clock_seconds:90,retries:0}};
}
