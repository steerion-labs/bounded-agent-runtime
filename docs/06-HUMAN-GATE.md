...
protected_actions: <sorted declared protected actions, shown to the human reviewer>
protected_actions_hash: <sha256 of the same sorted declared protected_actions>
...
After state reaches `ACCEPTED`, lease expiry alone does not invalidate the human decision. BAR still checks the lease generation and fencing-token hash against the authenticated `LEASE_ACQUIRED` journal evidence, and candidate, evidence and approval binding must remain unchanged.
