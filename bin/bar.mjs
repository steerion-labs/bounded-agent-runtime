...
try {
  ...
  else if (command === 'agents') { const agents = doctorReport().agents; console.log(has('--json') ? JSON.stringify(agents, null, 2) : Object.entries(agents).map(([n,a]) => `${a.installed ? 'OK' : '--'} ${n.padEnd(12)} ${a.roles.join('/')} ${a.executable || 'not found'} | version=${a.version || a.version_probe || 'unknown'} | ${a.boundary || 'controller-enforced'}`).join('\n')); }
