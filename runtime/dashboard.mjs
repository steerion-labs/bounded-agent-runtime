...
export function publicEvidence() {
  ...
  return (readJson(STATE_FILE).evidence || [])
    .map(item => ({ evidence_id: item.evidence_id, claim: item.claim, producer_identity: item.producer_identity, trust_class: item.trust_class, candidate_sha: item.candidate_sha, tree_hash: item.tree_hash, created_at: item.created_at, status: item.status }))
    .sort((a, b) => String(a.created_at || '').localeCompare(String(b.created_at || '')) || String(a.evidence_id || '').localeCompare(String(b.evidence_id || '')));
}
function html() {
  ...
  <script>const esc=value=>String(value??'').replace(/[&<>\"']/g,ch=>({'&':'&amp;','<':'&lt;','>':'&gt;','\"':'&quot;',"'":'&#39;'}[ch]));
  ...
  const flow=stages.map(x=>'<span class="step '+(x===s.state?'active':'')+'">'+esc(x)+'</span>').join('');
  document.querySelector('#root').innerHTML='<div class="grid"><div class="card"><div class="muted">State</div><div class="big">'+esc(s.state)+'</div></div><div class="card"><div class="muted">Evidence</div><div class="big">'+s.evidence_count+'</div></div><div class="card"><div class="muted">Human Gate</div><div class="big '+(s.human_gate_required?'warn':'ok')+'">'+(s.human_gate_required?'WAITING':'NO')+'</div></div><div class="card"><div class="muted">Doctor</div><div class="big">'+esc(d.status)+'</div></div></div><div class="flow">'+flow+'</div><div class="card"><h3>Candidate</h3><p><code>'+esc(s.candidate_sha||'not created')+'</code></p><p><code>'+esc(s.tree_hash||'')+'</code></p><h3>Workers</h3><p>'+esc(s.builder_adapter||'-')+' → '+esc(s.reviewer_adapter||'-')+'</p></div><div class="card" style="margin-top:14px"><h3>Evidence timeline</h3><table><thead><tr><th>Time</th><th>Status</th><th>Claim</th><th>Producer</th><th>Trust</th><th>Candidate</th></tr></thead><tbody>'+e.map(x=>'<tr><td>'+esc(x.created_at||'-')+'</td><td>'+esc(x.status||'-')+'</td><td>'+esc(x.claim)+'</td><td>'+esc(x.producer_identity)+'</td><td>'+esc(x.trust_class)+'</td><td><code>'+esc(x.candidate_sha||'-')+'</code><br><code>'+esc(x.tree_hash||'-')+'</code></td></tr>').join('')+'</tbody></table></div>'}
