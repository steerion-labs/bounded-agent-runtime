...
export function createFreshContextReview({ task_id, candidate_sha, tree_hash, builder_id, reviewer_id, evidence_refs = [] }) {
  ...
  const refs = evidence_refs.map(ref => requiredString(ref, 'BOUNDARY_REVIEW_EVIDENCE_REF_INVALID'));
  if (new Set(refs).size !== refs.length) throw new Error('BOUNDARY_REVIEW_EVIDENCE_REF_DUPLICATE');
  return Object.freeze({ task_id: taskId, candidate_sha: candidate, tree_hash: tree, builder_id: builder, reviewer_id: reviewer,
    evidence_refs: Object.freeze(refs), context_policy: 'FRESH_CONTEXT', workspace_mode: 'READ_ONLY', credential_mode: 'NONE', authority: 'NONE',
    may_mutate_candidate: false, may_approve_protected_action: false, verdicts: Object.freeze(['PASS','FAIL','NEEDS_EVIDENCE']) });
}
